...
exports.putTask = async (req, res) => {
  try {
    ...
    if (!title || !description || status === undefined) {
      return res.status(400).json({ status: false, msg: "Title, description, or status of task not found" });
    }
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { title, description, status }, { new: true });
